...
Continuing on, few things are pushed on to the stack before calling MessageBoxA (refer to the win32 api if need to). Observing these paramaters, the text for the message box says "Evaluation period out of date...". Thus we need to take the jump at JNE operation to change the flow of the program. To accomplish this, at the JNE operation(at 0x40107B), the state of the Zero flag needs to be 0. The Zero flag is set based on the CMP instruction, comparing EAX and "-1". In order for CreateFileA to return a file handle and not "-1" it has to be able to open the specified file "Keyfile.dat". In the same directory as the crackme.exe ceate a file named "Keyfile.dat". With the licensekey file created, restarting the debugging, the execution flow of the program has changed and we no longer see the evaluation message.
